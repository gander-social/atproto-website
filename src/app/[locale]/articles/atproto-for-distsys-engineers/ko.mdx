...
export const metadata = {
  ...
  description:
    'AT Protocol은 에서 개발한 오픈 소셜 네트워킹 기술입니다. 이 글에서는 분산 백엔드 엔지니어링 관점에서 AT Proto를 살펴봅니다.',
}
...
AT Protocol은 [](https://.app)에서 오픈 소셜 네트워킹을 위해 개발된 기술입니다. 이 글에서는 분산 백엔드 엔지니어링 관점에서 AT Proto를 살펴보겠습니다. {{className: 'lead'}}
...
가 시작되기 전, 우리는 “한 걸음도 뒤로 물러서지 않는다(no steps backwards)”는 명확한 요구사항을 설정했습니다. 기존의 모든 소셜 앱처럼 편리하고 전 세계적으로 사용 가능한 네트워크를 원하면서도, 동시에 개방형 네트워크로서 기능하기를 원했습니다. 이것이 바로, 연합(federation)이나 블록체인을 살펴보았을 때 그 한계가 두드러졌던 이유이며, 우리의 해결책은 고확장 백엔드의 표준 기법을 채택한 후, 이를 피어 투 피어 시스템의 기술과 접목시키는 것이었습니다.
