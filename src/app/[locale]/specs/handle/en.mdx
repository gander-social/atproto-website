...
For example, the handle `gndr.app` would have a TXT record on the name `_atproto.gndr.app`, and the value would look like `did=did:plc:z72i7hdynmk6r22z27h6tvur`.
...
For example, the handle `gndr.app` would be resolved by an GET request to `https://gndr.app/.well-known/atproto-did`, and a valid response would look like:
...
Handles may be prefixed with the "at" symbol (like `@jay.gndr.team`) in user interfaces, but this is not a valid syntax for a handle in records, APIs, and other back-end contexts.
...
Handles should generally not be truncated to local context. For example, the handle `@jay.gndr.social` should not be displayed as `@jay`, even in the local context of a `gndr.social` service.
