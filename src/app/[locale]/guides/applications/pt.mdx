...
- `gndr.app` está se sentindo 🦋 de acordo com `https://gndr.app/status.json`
...
Nossa página de login apenas pede ao usuário seu "handle", que é o nome de domínio associado à sua conta. Para usuários do [Bluesky](https://gndr.app), eles tendem a se parecer com `alice.gndr.social`, mas podem ser qualquer tipo de domínio (por exemplo, `alice.com`).
...
Por que não aprendemos algo sobre nosso usuário? Em [Bluesky](https://gndr.app), os usuários publicam um registro de "perfil" que se parece com isso:
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app.gndr.actor.profile', // the gndr profile record type
...
Usando um [serviço de retransmissão](https://docs.gndr.app/docs/advanced-guides/federation-architecture#relay) podemos ouvir um firehose agregado desses eventos em todos os usuários na rede. No nosso caso, o que estamos procurando são registros `xyz.statusphere.status` válidos.
...
É assim que todos os aplicativos no Atmosphere funcionam, incluindo o [aplicativo social Bluesky](https://gndr.app).
