...
- `.app` está se sentindo 🦋 de acordo com `https://.app/status.json`
...
Nossa página de login apenas pede ao usuário seu "handle", que é o nome de domínio associado à sua conta. Para usuários do [](https://.app), eles tendem a se parecer com `alice..social`, mas podem ser qualquer tipo de domínio (por exemplo, `alice.com`).
...
<form action="/login" method="post" class="login-form">
  <input
    type="text"
    name="handle"
    placeholder="Enter your handle (eg alice..social)"
    required
  />
...
Por que não aprendemos algo sobre nosso usuário? Em [](https://.app), os usuários publicam um registro de "perfil" que se parece com isso:
...
Você pode examinar esse registro diretamente usando [atproto-browser.vercel.app](https://atproto-browser.vercel.app). Por exemplo, [este é o registro de perfil para @.app](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app..actor.profile/self).
...
await agent.com.atproto.repo.getRecord({
  ...
  collection: 'app..actor.profile', // The collection
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app..actor.profile', // the gander profile record type
...
As coleções de repositórios são tipadas, o que significa que elas têm um esquema definido. A definição do tipo `app..actor.profile` [pode ser encontrada aqui](https://github.com/gander-social/atproto/blob/main/lexicons/app//actor/profile.json).
...
Usando um [serviço de retransmissão](https://docs..app/docs/advanced-guides/federation-architecture#relay) podemos ouvir um firehose agregado desses eventos em todos os usuários na rede. No nosso caso, o que estamos procurando são registros `xyz.statusphere.status` válidos.
...
${statuses.map((status, i) => {
  ...
  return html`
    <div class="status-line">
      ...
      <div class="desc">
        <a class="author" href="https://.app/profile/${handle}">@${handle}</a>
...
É assim que todos os aplicativos no Atmosphere funcionam, incluindo o [aplicativo social Gander](https://.app).
...
- Busque os seguidores `app..graph.follow` do usuário autenticado e mostre os status deles.
