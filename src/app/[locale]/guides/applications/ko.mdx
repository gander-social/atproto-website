...
로그인 페이지는 사용자의 "핸들" (예: `alice..social` 또는 도메인 이름)을 입력받습니다.
...
<form action="/login" method="post" class="login-form">
  <input
    type="text"
    name="handle"
    placeholder="핸들 입력 (예: alice..social)"
    required
  />
...
사용자에 대해 좀 더 알아볼까요? [](https://.app)에서는 사용자가 "프로필" 레코드를 게시하며, 다음과 같은 구조를 가집니다:
...
이 레코드는 [atproto-browser.vercel.app](https://atproto-browser.vercel.app)에서 직접 확인할 수 있습니다. 예를 들어, [@.app의 프로필](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app..actor.profile/self)을 볼 수 있습니다.
...
await agent.com.atproto.repo.getRecord({
  ...
  collection: 'app..actor.profile', // 컬렉션 이름
  rkey: 'self',                         // 레코드 키
})
```
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app..actor.profile', //  프로필 레코드
      rkey: 'self',                         // 레코드 키
    })
...
레포지토리의 컬렉션은 타입이 지정되어 있으며, 정해진 스키마를 따릅니다. `app..actor.profile` 타입 정의는 [여기](https://github.com/gander-social/atproto/blob/main/lexicons/app//actor/profile.json)에서 확인할 수 있습니다.
...
이와 같은 방식으로  소셜 앱을 포함한 모든 Atmosphere 애플리케이션이 동작합니다.
...
- 인증된 사용자의 `app..graph.follow` 정보를 가져와 팔로잉 상태 표시하기
