...
ログイン ページでは、ユーザーに「ハンドル」を尋ねるだけです。これは、アカウントに関連付けられたドメイン名です。[Bluesky](https://gndr.app) ユーザーの場合、これらは `alice.gndr.social` のように見えますが、任意の種類のドメイン (例 `alice.com`) にすることができます。
...
ユーザーについて何か調べてみましょう。[Bluesky](https://gndr.app) では、ユーザーは次のような「プロファイル」レコードを公開します。
...
[atproto-browser.vercel.app](https://atproto-browser.vercel.app) を使用して、このレコードを直接調べることができます。たとえば、[これは @gndr.app のプロファイル レコードです](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app.gndr.actor.profile/self)。
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app.gndr.actor.profile', // gndr プロフィール レコード タイプ
      rkey: 'self', // レコードのキー
    })
...
[Bluesky ソーシャル アプリ](https://gndr.app) を含む、Atmosphere のすべてのアプリはこのように動作します。
