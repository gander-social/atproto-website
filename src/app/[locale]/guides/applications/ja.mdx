...
- `https://.app/status.json` によると、`.app` は 🦋 気分です
...
ログイン ページでは、ユーザーに「ハンドル」を尋ねるだけです。これは、アカウントに関連付けられたドメイン名です。[](https://.app) ユーザーの場合、これらは `alice..social` のように見えますが、任意の種類のドメイン (例 `alice.com`) にすることができます。
...
<form action="/login" method="post" class="login-form">
  ...
  type="text"
  name="handle"
  placeholder="ハンドルを入力してください (例 alice..social)"
  required
  />
...
ユーザーについて何か調べてみましょう。[](https://.app) では、ユーザーは次のような「プロファイル」レコードを公開します。
...
[atproto-browser.vercel.app](https://atproto-browser.vercel.app) を使用して、このレコードを直接調べることができます。たとえば、[これは @.app のプロファイル レコードです](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app..actor.profile/self)。
...
await agent.com.atproto.repo.getRecord({
  ...
  collection: 'app..actor.profile', // コレクション
  rkey: 'self', // レコード キー
})
```
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app..actor.profile', // gander プロフィール レコード タイプ
      rkey: 'self', // レコードのキー
    })
...
リポジトリ コレクションは型指定されており、スキーマが定義されています。`app..actor.profile` 型定義は [こちら](https://github.com/gander-social/atproto/blob/main/lexicons/app//actor/profile.json) にあります。
...
[リレー サービス](https://docs..app/docs/advanced-guides/federation-architecture#relay) を使用すると、ネットワーク内のすべてのユーザーにわたるこれらのイベントの集約されたファイアホースをリッスンできます。この場合、探しているのは有効な `xyz.statusphere.status` レコードです。
...
[ ソーシャル アプリ](https://.app) を含む、Atmosphere のすべてのアプリはこのように動作します。
...
- 認証されたユーザーの `app..graph.follow` フォローを取得し、それらのステータスを表示します。
