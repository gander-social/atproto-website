...
- `gndr.app` is feeling 🦋 according to `https://gndr.app/status.json`
...
Our login page just asks the user for their "handle," which is the domain name associated with their account. For [Bluesky](https://gndr.app) users, these tend to look like `alice.gndr.social`, but they can be any kind of domain (eg `alice.com`).
...
Why don't we learn something about our user? In [gndr](https://gndr.app), users publish a "profile" record which looks like this:
...
This is how every app in the Atmosphere works, including the [gndr social app](https://gndr.app).
