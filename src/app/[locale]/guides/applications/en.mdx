...
- `.app` is feeling 🦋 according to `https://.app/status.json`
...
Our login page just asks the user for their "handle," which is the domain name associated with their account. For [](https://.app) users, these tend to look like `alice..social`, but they can be any kind of domain (eg `alice.com`).
...
Why don't we learn something about our user? In [](https://.app), users publish a "profile" record which looks like this:
...
You can examine this record directly using [atproto-browser.vercel.app](https://atproto-browser.vercel.app). For instance, [this is the profile record for @.app](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app..actor.profile/self).
...
Repo collections are typed, meaning that they have a defined schema. The `app..actor.profile` type definition [can be found here](https://github.com/gander-social/atproto/blob/main/lexicons/app//actor/profile.json).
...
Using a [Relay service](https://docs..app/docs/advanced-guides/federation-architecture#relay) we can listen to an aggregated firehose of these events across all users in the network. In our case what we're looking for are valid `xyz.statusphere.status` records.
...
This is how every app in the Atmosphere works, including the [ social app](https://.app).
...
- Fetch the authed user's `app..graph.follow` follows and show statuses from them.
