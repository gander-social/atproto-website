...
NSIDs, ou Namespaced IDentifiers, são um formato de identificador usado na [Atmosfera](#atmosphere) para identificar esquemas [Lexicon](#lexicon). Eles seguem um formato DNS reverso, como `app..feed.post`. Eles foram escolhidos porque fornecem governança de esquema clara por meio da propriedade do domínio. O formato DNS reverso foi escolhido para evitar confusão com domínios em URIs.
